...
//	PrivateKey hzfPriKey = TestUtil.getPrivateKey();
	...
	while (i<1) {
		...
		try {
			...
			sBuilder.append("<cycleValue>1</cycleValue>");  //1:T1 2:D0 
			sBuilder.append("<allotFlag>1</allotFlag>");    //0:不允许 1：允许
			...
			sBuilder.append("<futureRateValue>0.01</futureRateValue>");//费率类型为1时填费率的百分数；为2是填单笔金额
//			sBuilder.append("<futureMinAmount>0.01</futureMinAmount>");
//			sBuilder.append("<futureMaxAmount>15</futureMaxAmount>");
//            sBuilder.append("<attachRateType>2</attachRateType>");
